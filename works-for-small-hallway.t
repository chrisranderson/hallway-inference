...
function infer_scene(filename)
  local p2 = qs.program(function()
    ...
    return terra()
      ...
      var width = qs.uniform(IMAGE_WIDTH/5.0, IMAGE_WIDTH/2.0, {init=[init_width], struc=false})
      var height = 1.2*width
      var rotation = qs.uniform(-PI/2, PI/2, {init=[init_rotation], struc=false})
  ...
  -- HMC breaks quickly
  local infer = qs.infer(p2, qs.MAP, qs.MCMC(qs.TraceMHKernel(), {numsamps=2000, verbose=true}))
